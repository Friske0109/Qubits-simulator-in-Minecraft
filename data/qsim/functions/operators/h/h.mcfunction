...
#upperを2**targetに設定
scoreboard players operation p2n_input qubits = h_target qubits
scoreboard players set p2n_output qubits 1
function qsim:operators/p2n
scoreboard players operation h_upper qubits = p2n_output qubits
...
scoreboard players set p2n_output qubits 1
function qsim:operators/p2n
scoreboard players operation h_lower qubits = p2n_output qubits

#upperに演算する用の値を2**(n_bits-target)に設定 (lower*2)
scoreboard players operation h_upper_mul qubits = h_lower qubits
scoreboard players operation h_upper_mul qubits *= 2 qubits

#loopカウントの初期化
scoreboard players set h_upper_loop qubits 0
scoreboard players set h_lower_loop qubits 0

#X(n_bits,x_target)の行列を用意
function qsim:operators/h/h_upper_loop
